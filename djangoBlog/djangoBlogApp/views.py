...
class index(View):
    ...
    def get(self, request):
        ...
        paginator = Paginator(post, 5)  # Show 5 article per page
...
def getCategory(request, name):
    ...
    page = request.GET.get('page')
    total_article = paginator.get_page(page)
    return render(request, "category.html", {"post": total_article, "cat": cat, "categories":categories, "tags":tags})
...
def getProfile(request):
    if request.user.is_authenticated:
        ...
        if author_profile:
            ...
            return render(request, 'my_article.html', {"post": post, "user": authorUser})
        else:
            form = createAuthor(request.POST or None, request.FILES or None)
            if form.is_valid():
                instance = form.save(commit=False)
